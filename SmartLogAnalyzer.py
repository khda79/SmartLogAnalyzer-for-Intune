...
class SmartLogAnalyzerApp(tk.Tk):
    ...
    def _build_tab_summary(self):
        f = self._tab_summary
        self._lbl_analysis_title = tk.Label(
            f, text="Analysis Overview",
            bg=C_BG, fg=C_HEADER, font=FONT_TITLE)
        self._lbl_analysis_title.pack(anchor="w", padx=16, pady=(12, 4))
        ...
        self._kpi_errors   = self._kpi_card(kpi_row, "Errors",
                                             "---", C_TEXT_DIM,
                                             on_click=lambda: self._goto_evtx("error"))
        self._kpi_warnings = self._kpi_card(kpi_row, "Warnings",
                                             "---", C_TEXT_DIM,
                                             on_click=lambda: self._goto_evtx("warning"))
    ...
    def _kpi_card(self, parent, label, value, color, on_click=None):
        ...
        val_lbl.pack()
        lbl_sub = tk.Label(card, text=label, bg=C_SURFACE, fg=C_TEXT_DIM,
                 font=("Segoe UI", 8))
        lbl_sub.pack()
        if on_click:
            for w in (card, val_lbl, lbl_sub):
                w.configure(cursor="hand2")
                w.bind("<Button-1>", lambda e, cb=on_click: cb())
                w.bind("<Enter>", lambda e, fr=card: fr.configure(bg=C_PANEL))
                w.bind("<Leave>", lambda e, fr=card: fr.configure(bg=C_SURFACE))
        return val_lbl
    ...
    def _build_tab_mdm_diag(self):
        ...
        self._lbl_cab = tk.Label(
            cab_bar,
            text="No CAB file in this ZIP — click to extract if present",
            bg=C_BG, fg=C_TEXT_DIM, font=("Segoe UI", 8))
    ...
    def _build_mdm_diag_inner_nb(self):
        ...
        # ---- Tab 2 : Policies ----
        n_pol_real  = sum(1 for pol in p.managed_policies if pol['area'] != 'knobs')
        n_pol_knobs = sum(1 for pol in p.managed_policies if pol['area'] == 'knobs')
        tab_pol = tk.Frame(nb, bg=C_BG)
        nb.add(tab_pol, text=f"  Policies ({n_pol_real})  ")
        ...
        ent = tk.Entry(flt_pol, textvariable=self._pol_filter_mdm,
                       ...
                       font=FONT_BODY, relief="flat", borderwidth=4)
        ent.pack(side="left", padx=6, ipadx=4)
        # "Show knobs" checkbox — knobs are internal Windows power/energy settings
        self._show_knobs_var = tk.BooleanVar(value=False)
        if n_pol_knobs > 0:
            tk.Checkbutton(
                flt_pol, text=f"Show internal knobs ({n_pol_knobs})",
                variable=self._show_knobs_var,
                bg=C_BG, fg=C_TEXT_DIM, selectcolor=C_SURFACE,
                activebackground=C_BG, activeforeground=C_TEXT,
                font=("Segoe UI", 8),
                command=self._filter_mdm_policies
    ...
    def _filter_mdm_policies(self):
        ...
        show_knobs = getattr(self, '_show_knobs_var', None)
        show_knobs = show_knobs.get() if show_knobs else False
        shown = 0
        AREA_COLOURS = {
        ...
        for i, pol in enumerate(p.managed_policies):
            if pol['area'] == 'knobs' and not show_knobs:
                continue   # skip internal Windows power/energy knobs by default
        ...
        n_real  = sum(1 for p2 in p.managed_policies if p2['area'] != 'knobs')
        n_knobs = sum(1 for p2 in p.managed_policies if p2['area'] == 'knobs')
        if hasattr(self, '_pol_count_mdm'):
            if term:
                self._pol_count_mdm.configure(text=f"{shown} policies matching")
            elif show_knobs:
                self._pol_count_mdm.configure(
                    text=f"{shown} policies  ({n_knobs} internal knobs included)")
            elif n_real == 0 and n_knobs > 0:
                self._pol_count_mdm.configure(
                    text=f"No MDM-assigned policies — {n_knobs} internal knobs hidden  (check box to show)",
                    fg=C_TEXT_DIM)
            else:
                self._pol_count_mdm.configure(
                    text=f"{n_real} policies", fg=C_TEXT_DIM)
    ...
    def _build_tab_win11(self):
        """Build the Windows 11 Upgrade Readiness tab (shown only for Win10 devices)."""
        f = self._tab_win11
        for w in f.winfo_children():
            w.destroy()

        tk.Label(f, text="Windows 11 Upgrade Readiness",
                 bg=C_BG, fg=C_HEADER, font=FONT_TITLE
                 ).pack(anchor="w", padx=16, pady=(12, 0))

        # Source attribution — always visible
        tk.Label(f,
                 text=("\U0001f52c  Powered by Microsoft HardwareReadiness.ps1"
                       "  —  official signed script (MIT license)"
                       "  |  https://aka.ms/HWReadinessScript"),
                 bg=C_BG, fg=C_TEXT_DIM, font=("Segoe UI", 8),
                 ).pack(anchor="w", padx=20, pady=(0, 8))

        # Summary banner
        self._lbl_win11_summary = tk.Label(
            f, text="", bg=C_SURFACE, fg=C_TEXT,
            font=("Segoe UI", 12, "bold"), anchor="w", padx=16, pady=8)
        self._lbl_win11_summary.pack(fill="x", padx=16, pady=(0, 8))

        # Checklist treeview
        cols = ("check", "status", "value", "detail")
        self._tree_win11 = self._make_tree(
            f, cols,
            headings=["Requirement", "Status", "Current Value", "Detail"],
            widths=[220, 90, 260, 440])
        self._tree_win11.tag_configure("pass",    foreground=C_OK)
        self._tree_win11.tag_configure("fail",    foreground=C_ERROR)
        self._tree_win11.tag_configure("warn",    foreground=C_WARN)
        self._tree_win11.tag_configure("unknown", foreground=C_TEXT_DIM)
        self._tree_win11.tag_configure("section", foreground=C_ACCENT2,
                                       font=("Segoe UI", 9, "bold"))

        # Notes area
        tk.Label(f, text="Notes & Recommendations:",
                 bg=C_BG, fg=C_TEXT_DIM, font=FONT_BOLD
                 ).pack(anchor="w", padx=16, pady=(6, 0))
        self._txt_win11_notes = self._scrolled_text(f, height=7, mono=False)

    def _populate_win11_from_ms_json(self, obj):
        """
        Parse Microsoft's HardwareReadiness.ps1 JSON output and display results.
        JSON keys: returnCode (0=CAPABLE, 1=NOT CAPABLE, -1=UNDETERMINED, -2=FAILED TO RUN),
                   returnResult, returnReason, logging
        logging format:  "Storage: OSDiskSize=237GB. PASS; Memory: System_Memory=16GB. PASS;
                          TPM: TPMVersion=2.0, 2.0. PASS; Processor: {...}. FAIL; ..."
        """
        import re as _re

        code    = obj.get("returnCode",   -2)
        result  = obj.get("returnResult", "FAILED TO RUN")
        reason  = obj.get("returnReason", "").strip(", ")
        logging = obj.get("logging",      "")

        # ── Parse the logging string into individual check results ─────────
        # Each segment: "CheckName: detail. PASS/FAIL/UNDETERMINED; "
        checks = []
        DISPLAY_NAMES = {
            "Storage":    ("Disk ≥ 64 GB",      "OS disk total size"),
            "Memory":     ("RAM ≥ 4 GB",         "Physical memory"),
            "TPM":        ("TPM 2.0",            "Trusted Platform Module version"),
            "Processor":  ("CPU Compatible",     "Processor family & speed"),
        ...
        failed_checks = set(r.strip().rstrip(",") for r in reason.split(",") if r.strip())

        # Split on "; " boundaries, keeping each segment
        segments = _re.findall(
            r'([A-Za-z0-9_ ()-]+?):\s*(.*?)\.\s*(PASS|FAIL|UNDETERMINED|FAILED TO RUN)\s*;?\s*',
            logging)
        for (chk_raw, detail, verdict) in segments:
            chk = chk_raw.strip()
            name, desc = DISPLAY_NAMES.get(chk, (chk, ""))
            if verdict == "PASS":
                status = "pass"
            elif verdict == "FAIL":
                status = "fail"
                ...
                status = "unknown"
            # Extract value from detail (e.g. "OSDiskSize=237GB" → "237 GB")
            val_m = _re.search(r'=([^.;{]+)', detail)
            value = val_m.group(1).strip() if val_m else detail.strip()[:60]
            checks.append((name, status, value, detail.strip()[:120]))

        if not checks:
            # Logging parse failed — show raw
            checks.append(("Microsoft Check", "unknown", result, logging[:200]))

        # ── Populate treeview ─────────────────────────────────────────────
        STATUS_ICON = {"pass": "✓  PASS", "fail": "✗  FAIL",
                       "warn": "⚠  CHECK", "unknown": "?  UNDETERMINED"}
        for i, (req, status, value, detail) in enumerate(checks):
            self._tree_win11.insert(
                "", "end",
                values=(req, STATUS_ICON.get(status, status), value, detail),
                tags=(status, "even" if i % 2 == 0 else "odd"))

        # ── Summary banner ─────────────────────────────────────────────────
        if code == 0:
            summary = "✓  CAPABLE — Device meets all Windows 11 requirements"
            s_color = C_OK
        elif code == 1:
            blockers = reason if reason else "see details above"
            summary  = f"✗  NOT CAPABLE — Blocker(s): {blockers}"
            s_color  = C_ERROR
        elif code == -1:
            summary  = "⚠  UNDETERMINED — Some checks could not be completed"
            s_color  = C_WARN
        else:
        ...
        # ── Notes ──────────────────────────────────────────────────────────
        note_lines = [
            "Source: Microsoft HardwareReadiness.ps1 (official signed script — MIT license)\n",
            f"Result : {result}  (returnCode={code})\n",
        ]
        if reason:
            note_lines.append(f"Blockers: {reason}\n")
        if code == 1:
            note_lines += [
                "\nAction required for each blocker:",
                "  • TPM: Enable TPM 2.0 in BIOS/UEFI settings",
                "  • Secure Boot: Switch to UEFI mode and enable Secure Boot",
                "  • Storage: OS drive must be ≥ 64 GB total (not free space)",
                "  • Processor: CPU must be on Microsoft's supported list (gen 8+ Intel / Zen 2+ AMD)",
                "  • Memory: Install at least 4 GB RAM",
            ]
        note_lines.append(
            "\nFull script log:\n" + logging)
    ...
    def _populate_win11_readiness(self):
        ...
        JSON output if available (local scan), otherwise falls back to manual checks."""
        import re as _re, json as _json

        self._tree_clear(self._tree_win11)
        self._txt_win11_notes.configure(state="normal")
        self._txt_win11_notes.delete("1.0", "end")

        # ── Try official Microsoft JSON first (local scan only) ────────────
        _json_file = self._zip_handler.find_file("win11_readiness")
        if _json_file and os.path.isfile(_json_file):
            try:
                with open(_json_file, encoding="utf-8") as _f:
                    obj = _json.load(_f)
                self._populate_win11_from_ms_json(obj)
                return
            except Exception:
                pass   # fall through to manual checks

        checks = []   # list of (requirement, status, current_value, detail)
        notes  = []

        # ── Helper ─────────────────────────────────────────────────────────
        def _read_ext(keyword):
            _f = self._zip_handler.find_file(keyword)
            if not _f:
                return ""
            try:
        ...
        # ── OS Build ───────────────────────────────────────────────────────
        di   = self._mdm_parser.device_info if hasattr(self._mdm_parser, 'device_info') else {}
        si   = _read_ext("ps_system_info")
        _os_build_str = di.get("OS Build", "")
        _m_bld = _re.search(r'(?:OsBuildNumber|OsVersion)\s*:\s*([\d.]+)', si)
        if not _os_build_str and _m_bld:
            _os_build_str = _m_bld.group(1)
        try:
            _build_num = int(_os_build_str.split(".")[2]) if "." in _os_build_str else int(_os_build_str)
        except Exception:
            _build_num = 0

        # ── 1. TPM 2.0 ────────────────────────────────────────────────────
        # dsregcmd TpmProtected=YES means TPM IS protecting creds (implies TPM 2.0 capable)
        # TpmProtected=NO doesn't confirm absence — TPM may exist but not used for creds
        dsreg_wa = {}
        for sname in ("Work Account 1", "Work Account 2", "Device Details"):
            s = self._mdm_parser.dsregcmd.sections.get(sname, {})
            if s.get("TpmProtected"):
                dsreg_wa = s
                break
        tpm_prot = dsreg_wa.get("TpmProtected", "").upper()
        if tpm_prot == "YES":
            checks.append(("TPM 2.0", "pass", "Protected (YES)",
                           "TPM is actively protecting credentials — TPM 2.0 likely present"))
        elif tpm_prot == "NO":
            checks.append(("TPM 2.0", "warn", "Not protecting (NO)",
                           "TPM exists but not protecting creds — may be TPM 1.2 or disabled in BIOS"))
            notes.append("• TPM: Enable TPM 2.0 in BIOS/UEFI and ensure it is selected as active PCR.")
        else:
            checks.append(("TPM 2.0", "unknown", "Unknown",
                           "Could not determine TPM state from available data"))

        # ── 2. Secure Boot ────────────────────────────────────────────────
        sb_text = _read_ext("ps_secureboot")
        _sb_m = _re.search(r'SecureBootEnabled\s*:\s*(\S+)', sb_text)
        sb_val = _sb_m.group(1).strip() if _sb_m else ""
        if sb_val.lower() in ("true", "1", "yes"):
            checks.append(("Secure Boot", "pass", "Enabled", "Secure Boot is active"))
        elif sb_val.lower() in ("false", "0", "no"):
            checks.append(("Secure Boot", "fail", "DISABLED",
                           "Secure Boot must be enabled for Windows 11"))
            notes.append("• Secure Boot: Enable in BIOS/UEFI → Security → Secure Boot → Enabled. "
                         "May require switching from Legacy/CSM to UEFI boot mode.")
        else:
            checks.append(("Secure Boot", "unknown", "Unknown",
                           "ps_secureboot.txt not found or unreadable"))

        # ── 3. CPU Compatibility ──────────────────────────────────────────
        cpu_str = di.get("Processor", "")
        if not cpu_str:
            _m = _re.search(r'CsProcessors\s*:\s*\{?([^}]+)\}?', si)
            cpu_str = _m.group(1).strip() if _m else ""

        def _check_cpu_win11(cpu):
            cpu_u = cpu.upper()
            # Intel: Core i3/i5/i7/i9 — need 8th gen or later (model number starts with 8+)
            # Pattern: "Core(TM) iX-NNNN" where N is gen digit
            m_intel = _re.search(r'CORE.*?I[3579]-(\d)(\d{3})', cpu_u)
            if m_intel:
                gen = int(m_intel.group(1))
                if gen >= 8:
                    return "pass", f"Intel Core gen {gen} — compatible"
                else:
                    return "fail", f"Intel Core gen {gen} — NOT compatible (needs gen 8+)"
            # Intel Xeon Scalable (3rd gen+), Pentium Gold G6000+, Celeron G5000+
            if "XEON" in cpu_u:
                return "warn", "Intel Xeon — check specific model against MS list"
            # AMD Ryzen: Ryzen 3000+ (Zen 2) or Ryzen Pro 3000+
            m_amd = _re.search(r'RYZEN\s+\d+\s+(\d)(\d{3})', cpu_u)
            if m_amd:
                gen_digit = int(m_amd.group(1))
                if gen_digit >= 3:
                    return "pass", f"AMD Ryzen {gen_digit}xxx — compatible (Zen 2+)"
                else:
                    return "fail", f"AMD Ryzen {gen_digit}xxx — NOT compatible (needs 3000+)"
            return "unknown", f"CPU not recognized: {cpu[:60]}"

        cpu_status, cpu_detail = _check_cpu_win11(cpu_str)
        checks.append(("CPU Compatibility", cpu_status, cpu_str[:55], cpu_detail))
        if cpu_status == "fail":
            notes.append(f"• CPU: {cpu_detail}. This is a hardware blocker — "
                         "Windows 11 cannot be installed without a compatible CPU.")

        # ── 4. RAM ≥ 4 GB ─────────────────────────────────────────────────
        ram_gb = 0
        ram_str = di.get("Installed RAM", "")
        if not ram_str:
            _m = _re.search(r'CsTotalPhysicalMemory\s*:\s*(\d+)', si)
            if _m:
                ram_gb = int(_m.group(1)) // 1_073_741_824
                ram_str = f"{ram_gb} GB"
        else:
            _m = _re.search(r'([\d.]+)', ram_str)
            if _m:
                ram_gb = float(_m.group(1))
        if ram_gb >= 4:
            checks.append(("RAM ≥ 4 GB", "pass", ram_str, f"{ram_gb:.0f} GB installed — meets requirement"))
        elif ram_gb > 0:
            checks.append(("RAM ≥ 4 GB", "fail", ram_str, f"Only {ram_gb:.0f} GB — minimum is 4 GB"))
            notes.append("• RAM: Add memory to reach at least 4 GB.")
        else:
            checks.append(("RAM ≥ 4 GB", "unknown", "Unknown", "RAM info not available"))

        # ── 5. Storage ≥ 64 GB free on system drive ───────────────────────
        disk_text = _read_ext("ps_disk_usage")
        c_free_gb = 0
        c_total_gb = 0
        for dline in disk_text.splitlines():
            _dp = dline.split()
            if len(_dp) >= 4 and _dp[0].upper() == "C":
                try:
                    c_free_gb  = int(_dp[2]) // 1_073_741_824
                    c_total_gb = int(_dp[3]) // 1_073_741_824
                except Exception:
                    pass
                break
        if c_free_gb >= 64:
            checks.append(("Disk C: ≥ 64 GB free", "pass",
                           f"{c_free_gb} GB free / {c_total_gb} GB",
                           "Sufficient free space for upgrade"))
        elif c_free_gb > 0:
            checks.append(("Disk C: ≥ 64 GB free", "fail",
                           f"Only {c_free_gb} GB free / {c_total_gb} GB",
                           "Minimum 64 GB free required for in-place upgrade"))
            notes.append(f"• Disk: Only {c_free_gb} GB free on C:. "
                         "Free at least 64 GB before upgrading (delete temp files, move data).")
        else:
            checks.append(("Disk C: ≥ 64 GB free", "unknown", "Unknown",
                           "ps_disk_usage.txt not found"))

        # ── 6. UEFI Firmware ─────────────────────────────────────────────
        # If SecureBoot is enabled → UEFI is confirmed. If disabled, UEFI may still be present.
        if sb_val.lower() in ("true", "1", "yes"):
            checks.append(("UEFI Firmware", "pass", "UEFI confirmed",
                           "Secure Boot enabled implies UEFI firmware"))
        elif sb_val.lower() in ("false", "0", "no"):
            checks.append(("UEFI Firmware", "warn", "Secure Boot off — verify UEFI mode",
                           "Device may be in Legacy/CSM boot mode — check BIOS settings"))
        else:
            checks.append(("UEFI Firmware", "unknown", "Unknown",
                           "Cannot confirm UEFI mode without Secure Boot data"))

        # ── Display + Internet (informational) ───────────────────────────
        checks.append(("Display ≥ 720p", "warn", "Not checked",
                       "Requires 9\" diagonal, 720p resolution — verify manually"))
        checks.append(("Internet Connection", "warn", "Not checked",
                       "Required for Windows 11 Home setup — Enterprise can bypass"))

        # ── Populate treeview ────────────────────────────────────────────
        STATUS_ICON = {"pass": "✓  PASS", "fail": "✗  FAIL",
                       "warn": "⚠  CHECK", "unknown": "?  UNKNOWN"}
        for i, (req, status, value, detail) in enumerate(checks):
            self._tree_win11.insert(
                "", "end",
                values=(req, STATUS_ICON.get(status, status), value, detail),
        ...
        if n_fail == 0 and n_pass >= 4:
            summary = f"✓  LIKELY COMPATIBLE — {n_pass}/{n_total} requirements met"
            s_color = C_OK
        elif n_fail > 0:
            summary = f"✗  BLOCKED — {n_fail} requirement(s) NOT met  ({n_pass}/{n_total} pass)"
            s_color = C_ERROR
        else:
        ...
        # ── Notes ─────────────────────────────────────────────────────────
        self._txt_win11_notes.insert("end",
            "\u26a0  Analyse statique basee sur les donnees disponibles dans le ZIP Intune.\n"
            "   Pour un resultat officiel certifie, utilisez 'Collect diagnostics' depuis l'app,\n"
            "   qui executera HardwareReadiness.ps1 (script officiel signe Microsoft, MIT license).\n\n")
        if notes:
            self._txt_win11_notes.insert("end",
    ...
    def _analysis_thread(self):
        try:
            ...
            if not self._device_os:
                ...
                for section in self._mdm_parser.dsregcmd.sections.values():
                    ...
                    if v:
                        ...
                        break
            if not self._device_os:
                # Fallback: read from extended/ps_system_info.txt (local scan)
                _si = self._zip_handler.find_file("ps_system_info")
                if _si and os.path.isfile(_si):
                    try:
                        with open(_si, encoding="utf-8", errors="replace") as _f:
                            _si_text = _f.read()
                        _m_os = re.search(r'OsName\s*:\s*(.+)', _si_text)
                        _m_bld = re.search(r'OsBuildNumber\s*:\s*(\d+)', _si_text)
                        if _m_os:
                            self._device_os = _m_os.group(1).strip()
                            if _m_bld:
                                self._device_os += f" (build {_m_bld.group(1).strip()})"
                    except Exception:
                        pass
            ...
            # Hardware & Security: battery, firewall, certificates
            ...
            # Health analysis
            self._set_status("Running health diagnostics...")
            try:
                ha = HealthAnalyzer()
                # Collect AAD event log text for Office Auth analysis
                _aad_txt = ""
                for _ef in list(inventory.get("evtx", {}).values())[:20]:
                    if "aad" in str(_ef).lower() or "microsoft-windows-aad" in str(_ef).lower():
                        try:
                            with open(_ef, encoding="utf-8", errors="replace") as _f2:
                                _aad_txt = _f2.read()[:50000]
                        except Exception:
                            pass
                # Resolve extended/ folder from ZIP extraction path
                _ext_dir = ""
                _all_files = inventory.get("all_files", [])
    ...
    def _populate_ui_s6(self):
        ...
        self._populate_wu()
        self._maybe_show_win11_tab()
        self.after(10, self._populate_ui_s7)

    def _maybe_show_win11_tab(self):
        """Add Win11 Readiness tab if device is Windows 10 (build < 22000)."""
        import re as _re
        di = getattr(self._mdm_parser, 'device_info', {})
        si_file = self._zip_handler.find_file("ps_system_info")
        si_text = ""
        if si_file:
            try:
                with open(si_file, encoding="utf-8", errors="replace") as _f:
                    si_text = _f.read()
            except Exception:
                pass
        build_str = di.get("OS Build", "")
        if not build_str:
            _m = _re.search(r'OsBuildNumber\s*:\s*(\d+)', si_text)
            if _m:
                build_str = _m.group(1)
        try:
            build_num = int(build_str.split(".")[2]) if "." in build_str else int(build_str)
        except Exception:
            build_num = 0
        # Windows 11 starts at build 22000 — show tab only for Windows 10
        is_win10 = 0 < build_num < 22000
        ...
        if is_win10 and not already_added:
            ...
            self._build_tab_win11()
            self._populate_win11_readiness()
        elif not is_win10 and already_added:
            # Device is Win11 — hide the tab if present
            try:
                self._nb.forget(self._tab_win11)
            except Exception:
                pass
    ...
    def _populate_ui_s7(self):
        ...
        self.after(10, self._populate_ui_s7b)

    def _populate_ui_s7b(self):
        self._set_status("Updating UI: Health...")
    ...
    def _analysis_done_ui(self):
        ...
        if self._analysis_done and self._extra_parser.cab_files:
            ...
            # Fallback: look for MDM Diag HTML directly (no CAB needed).
            # Intune portal CAB  → "MDMDiagHTMLReport.html"
            # Local MDMDiagnosticsTool -out → "MDMDiagReport.html" or other HTML
            _all = self._zip_handler.file_inventory.get("all_files", [])
            _html_direct = (
                self._zip_handler.find_file("mdmdiaghtmlreport") or
                self._zip_handler.find_file("mdmdiagreport") or
                next((f for f in _all
                      if "mdmdiag" in f.replace("\\", "/").lower()
                      and f.lower().endswith(".html")), None)
            )
            if _html_direct and os.path.isfile(_html_direct):
                self._mdm_diag_parser = MDMDiagParser()
                self._mdm_diag_parser.parse(_html_direct)
                _hname = os.path.basename(_html_direct)
                self._lbl_cab.configure(
                    text=f"{_hname} parsed directly (no CAB required)",
                    fg=C_OK)
                self._btn_cab.configure(state="disabled")
                self.after(200, self._build_mdm_diag_inner_nb)
                self.after(210, self._update_kpis)
            else:
                # Show what MDMDiagnosticsTool actually created in mdmdiag/
                _mdmdiag_files = [
                    f for f in _all
                    if "mdmdiag" in f.replace("\\", "/").lower()
                ]
                if _mdmdiag_files:
                    _info = "MDMDiagnosticsTool output (no HTML found):\n"
                    for _mf in sorted(_mdmdiag_files)[:30]:
                        try:
                            _sz = os.path.getsize(_mf)
                            _sz_s = (f"{_sz//1024} KB" if _sz > 1024 else f"{_sz} B")
                        except Exception:
                            _sz_s = "?"
                        _info += f"  {os.path.basename(_mf):50s}  {_sz_s}\n"
                    self._mdm_diag_placeholder.configure(
                        text=_info, justify="left", font=("Consolas", 8))
        ...
        # Update CAB label with actual status
        if self._analysis_done:
            _cabs = list(self._zip_handler.file_inventory.get("cab", []))
            _all2   = self._zip_handler.file_inventory.get("all_files", [])
            _html_d = (
                self._zip_handler.find_file("mdmdiaghtmlreport") or
                self._zip_handler.find_file("mdmdiagreport") or
                next((f for f in _all2
                      if "mdmdiag" in f.replace("\\", "/").lower()
                      and f.lower().endswith(".html")), None)
            )
            if _cabs:
                _cn = os.path.basename(_cabs[0])
                self._lbl_cab.configure(
                    text=f"CAB found: {_cn} — extracting...", fg=C_TEXT_DIM)
                self._btn_cab.configure(state="normal")
            elif _html_d and os.path.isfile(_html_d):
                pass  # will be updated by the fallback handler above
            else:
                self._lbl_cab.configure(
                    text="No CAB and no MDMDiagHTMLReport.html in this ZIP",
                    fg=C_TEXT_DIM)
                self._btn_cab.configure(state="disabled")
    ...
    def _goto_evtx(self, level_filter):
        """Switch to Event Log tab and apply level filter (error / warning)."""
        # 1. Select the Event Log tab
        self._nb.select(self._tab_eventlog)

        # 2. Apply the level filter if the inner notebook is built
        widgets = self._evtx_widgets.get("__shared__")
        if not widgets:
            return
        lv_var   = widgets["lv_var"]
        tree     = widgets["tree"]
        count_lbl = widgets["count_lbl"]

        lv_var.set(level_filter)

        # Determine the currently selected log_type from the combo
        # (we re-filter whatever channel is active)
        # Walk evtx_widgets to find a real log_type (not __shared__)
        log_type = next(
            (k for k in self._evtx_widgets if k != "__shared__"), "")
        if log_type:
            self._filter_evtx_events(log_type, level_filter, tree, count_lbl)
    ...
    def _populate_summary(self):
        ...
        _src_label = ("Local Device Scan"
                      if getattr(self, "_is_local_scan", False)
                      else "Intune DiagLogs ZIP")
        lines = [
            "=" * 64,
        ...
        # ── Disk space alerts ─────────────────────────────────────────────────
        _disk_file = self._zip_handler.find_file("ps_disk_usage")
        if _disk_file and os.path.isfile(_disk_file):
            _disk_alerts = []
            try:
                with open(_disk_file, encoding="utf-8", errors="replace") as _df:
                    _disk_lines = _df.readlines()
                # Format-Table -AutoSize output:
                #   Name  Used        Free        Total       Free%
                #   ----  ----        ----        -----       -----
                #   C     47681486848 2281312256  49962799104  4.6
                _hcols = []
                for _dline in _disk_lines:
                    _s = _dline.strip()
                    if not _s or _s.startswith("#"):
                        continue
                    if "Name" in _s and "Free%" in _s:
                        _hcols = _s.split()
                        continue
                    if not _hcols or _s.startswith("-"):
                        continue
                    _parts = _s.split()
                    if len(_parts) < len(_hcols):
                        continue
                    try:
                        _i_name  = _hcols.index("Name")
                        _i_free  = _hcols.index("Free")
                        _i_total = _hcols.index("Total")
                        _i_pct   = _hcols.index("Free%")
                        _dname   = _parts[_i_name]
                        _pct     = float(_parts[_i_pct])
                        _free_b  = _parts[_i_free]
                        _tot_b   = _parts[_i_total]
                        _free_gb  = round(int(_free_b) / 1_073_741_824, 1) \
                                    if _free_b.isdigit() else None
                        _total_gb = round(int(_tot_b)  / 1_073_741_824, 1) \
                                    if _tot_b.isdigit()  else None
                        _sz = (f"{_free_gb} GB free / {_total_gb} GB total"
                               if _free_gb is not None else f"{_pct}% free")
                        if _pct < 10:
                            _disk_alerts.append(("!!", _dname, _pct, _sz, "CRITICAL"))
                        elif _pct < 20:
                            _disk_alerts.append(("!", _dname, _pct, _sz, "LOW"))
                    except (ValueError, IndexError):
                        continue
            except Exception:
                pass
            if _disk_alerts:
                lines += ["-" * 64, "DISK SPACE ALERTS:"]
                for _sev, _dn, _pct, _sz, _lbl in _disk_alerts:
                    lines.append(f"  [{_sev}] Drive {_dn}:  {_pct}% free  ({_sz})  [{_lbl}]")
                lines.append("")
        ...
        # ── Intune enrollment status (local scan only) ────────────────────────
        enroll_check = self._zip_handler.find_file("intune_enrollment_check")
        if enroll_check and os.path.isfile(enroll_check):
            try:
                with open(enroll_check, encoding="utf-8", errors="replace") as _f:
                    _ec = _f.read()
                # Show a prominent banner based on the detected status
                if "NOT ENROLLED" in _ec:
                    lines += ["-" * 64,
                              "[!!] INTUNE ENROLLMENT: NOT ENROLLED",
                              "     Troubleshooting report available — see section below.",
                              ""]
                elif "ENROLLED" in _ec:
                    lines += ["-" * 64,
                              "[OK] INTUNE ENROLLMENT: ENROLLED",
                              ""]
                # Append the full report
                lines += ["-" * 64, "INTUNE ENROLLMENT REPORT:"]
                lines.append(_ec)
                lines.append("")
            except Exception:
                pass
        ...
        # ── Update "Analysis Overview" title with source + enrollment badge ──
        _src = "Local Scan" if getattr(self, "_is_local_scan", False) else "Intune ZIP"
        # Determine enrollment status from the enrollment check file
        _enroll_badge = ""
        _enroll_color = C_HEADER
        _enroll_file  = self._zip_handler.find_file("intune_enrollment_check")
        if _enroll_file and os.path.isfile(_enroll_file):
            try:
                with open(_enroll_file, encoding="utf-8", errors="replace") as _ef:
                    _ec2 = _ef.read()
                if "NOT ENROLLED" in _ec2:
                    _enroll_badge = "  ✗ Not Enrolled"
                    _enroll_color = C_ERROR
                elif "ENROLLED" in _ec2:
                    _enroll_badge = "  ✓ Enrolled"
                    _enroll_color = C_OK
            except Exception:
                pass
        _title_text = f"Analysis Overview  [{_src}]{_enroll_badge}"
        _title_color = _enroll_color if _enroll_badge else C_HEADER
        if hasattr(self, "_lbl_analysis_title"):
            self._lbl_analysis_title.configure(
                text=_title_text, fg=_title_color)

        # ── Also update the filename bar with enrollment badge ────────────────
        if _enroll_badge and hasattr(self, "_lbl_file") and self._zip_path:
            _fname = os.path.basename(self._zip_path)
            _src_tag = "  (local device)" if getattr(self, "_is_local_scan", False) else ""
            _file_badge = "  ✓ Enrolled" if "Enrolled" in _enroll_badge and "Not" not in _enroll_badge \
                          else "  ✗ Not Enrolled"
            self._lbl_file.configure(
                text=f"  {_fname}{_src_tag}{_file_badge}",
                fg=_enroll_color)
    ...
    def _populate_device_info(self):
        ...
        rows = []  # list of (section_label, key, value)

        # ── Hardware (from MDM Diagnostics + ps_system_info) ──────────────────
        di = self._mdm_parser.device_info if hasattr(self._mdm_parser, 'device_info') else {}
        si_text = ""
        _si_file = self._zip_handler.find_file("ps_system_info")
        if _si_file:
            try:
                with open(_si_file, encoding="utf-8", errors="replace") as _f:
                    si_text = _f.read()
            except Exception:
                pass
        ...
        def _si(key):
            import re as _re
            m = _re.search(rf'{key}\s*:\s*(.+)', si_text)
            return m.group(1).strip() if m else ""

        rows += [
            ("Hardware", "Computer Name",   di.get("PC name", "") or _si("CsName")),
            ("Hardware", "Manufacturer",    _si("CsManufacturer") or ""),
            ("Hardware", "Model",           _si("CsModel") or ""),
            ("Hardware", "Processor",       di.get("Processor", "") or _si("CsProcessors").strip("{}")),
            ("Hardware", "Installed RAM",   di.get("Installed RAM", "") or
                         (f"{int(_si('CsTotalPhysicalMemory'))//1073741824} GB"
                          if _si("CsTotalPhysicalMemory").isdigit() else "")),
            ("Hardware", "System Type",     di.get("System Type", "")),
            ("Hardware", "BIOS Version",    _si("BiosBIOSVersion").strip("{}")),
            ("Hardware", "BIOS Manufacturer", _si("BiosManufacturer")),
        ]

        # ── OS ─────────────────────────────────────────────────────────────────
        rows += [
            ("OS", "Edition",        di.get("Edition", "") or _si("OsName")),
            ("OS", "OS Build",       di.get("OS Build", "") or _si("OsVersion")),
            ("OS", "Architecture",   _si("OsArchitecture")),
            ("OS", "Language",       _si("OsLanguage")),
            ("OS", "Last Boot",      _si("OsLastBootUpTime")),
            ("OS", "Hypervisor",     _si("HyperVisorPresent")),
        ]

        # ── Security ───────────────────────────────────────────────────────────
        sb_text = ""
        _sb_file = self._zip_handler.find_file("ps_secureboot")
        if _sb_file:
            try:
                with open(_sb_file, encoding="utf-8", errors="replace") as _f:
                    sb_text = _f.read()
            except Exception:
                pass
        import re as _re
        _sb_m = _re.search(r'SecureBootEnabled\s*:\s*(\S+)', sb_text)
        secure_boot = _sb_m.group(1) if _sb_m else ""
        ep = getattr(self, '_extra_parser', None)
        bl_status = ep.ime_reg.bitlocker if ep else ""
        dsreg_wa = {}
        for sname in ("Work Account 1", "Work Account 2"):
            s = self._mdm_parser.dsregcmd.sections.get(sname, {})
            if s:
                dsreg_wa = s
                break
        tpm_protected = dsreg_wa.get("TpmProtected", "")
        rows += [
            ("Security", "Secure Boot",        secure_boot),
            ("Security", "TPM Protected",      tpm_protected),
            ("Security", "BitLocker",          bl_status),
        ]

        # ── MDM / Enrollment ───────────────────────────────────────────────────
        ci = self._mdm_parser.connection_info if hasattr(self._mdm_parser, 'connection_info') else {}
        ime_ver = ep.ime_reg.agent_version if ep else ""
        rows += [
            ("MDM",  "Managed by",              ci.get("Managed by", "")),
            ("MDM",  "Last MDM Sync",           ci.get("Last sync", "")),
            ("MDM",  "Management Server",       ci.get("Management server address", "")),
            ("MDM",  "IME Agent Version",       ime_ver),
        ]

        # ── Join State ─────────────────────────────────────────────────────────
        dev_s  = self._mdm_parser.dsregcmd.sections.get("Device State", {})
        usr_s  = self._mdm_parser.dsregcmd.sections.get("User State", {})
        sso_s  = self._mdm_parser.dsregcmd.sections.get("SSO State", {})
        rows += [
            ("Join State", "AAD Joined",        dev_s.get("AzureAdJoined", "")),
            ("Join State", "Workplace Joined",  dev_s.get("WorkplaceJoined", "") or usr_s.get("WorkplaceJoined", "")),
            ("Join State", "Domain Joined",     dev_s.get("DomainJoined", "")),
            ("Join State", "Azure AD PRT",      sso_s.get("AzureAdPrt", "")),
            ("Join State", "WAM Default Set",   sso_s.get("WamDefaultSet", "") or usr_s.get("WamDefaultSet", "")),
        ]
        for sname in ("Work Account 1", "Work Account 2"):
            s = self._mdm_parser.dsregcmd.sections.get(sname, {})
            if s:
                tenant = s.get("WorkplaceTenantName", sname)
                cert_v = s.get("DeviceCertificateValidity", "")
                rows += [
                    (f"Join: {tenant}", "Workplace Device ID", s.get("WorkplaceDeviceId", "")),
                    (f"Join: {tenant}", "Cert Validity",       cert_v),
                    (f"Join: {tenant}", "TPM Protected",       s.get("TpmProtected", "")),
                ]
        ...
        if _disk_file:
            try:
                with open(_disk_file, encoding="utf-8", errors="replace") as _f:
                    disk_text = _f.read()
                for dline in disk_text.splitlines():
                    _dp = dline.split()
                    if len(_dp) >= 5 and _dp[0].isalpha() and len(_dp[0]) == 1:
                        name, used, free, total, pct = _dp[0], _dp[1], _dp[2], _dp[3], _dp[4]
                        try:
                            free_gb  = int(free)  // 1_073_741_824
                            total_gb = int(total) // 1_073_741_824
                            rows.append(("Disk", f"Drive {name}:",
                                         f"{free_gb} GB free / {total_gb} GB total  ({pct.replace(',','.')}% free)"))
                        except Exception:
                            pass
            except Exception:
                pass
        ...
        zi = self._zip_info
        rows += [
            ("ZIP", "File",        zi.get("zip_name", "")),
            ("ZIP", "Size",        f"{zi.get('zip_size_mb','')} MB"),
            ("ZIP", "Total Files", str(zi.get("total_files", ""))),
        ]

        # ── Populate tree with section separators ──────────────────────────────
        _SECTION_COLORS = {
            "Hardware":   "#7eb8f7",
            "OS":         "#a0cfa0",
            "Security":   C_WARN,
        ...
        for (section, key, value) in rows:
            ...
            if section != last_section:
                self._tree_device.insert(
                    "", "end", values=(f"── {section} ──", ""),
                    tags=("section_hdr",))
                color = _SECTION_COLORS.get(section, C_TEXT)
                self._tree_device.tag_configure(
                    "section_hdr", foreground=color,
                    font=("Segoe UI", 9, "bold"))
                last_section = section
                row_idx = 0
            self._tree_device.insert(
                "", "end", values=(f"  {key}", value),
    ...
    def _build_tab_health_placeholder(self):
        """Initial placeholder — replaced by _build_health_tab() after analysis."""
        f = self._tab_health
        for w in f.winfo_children():
            w.destroy()
        tk.Label(f, text="\U0001fa7a  Security & Health Analysis",
                 bg=C_BG, fg=C_ACCENT, font=("Segoe UI", 14, "bold")).pack(pady=(30, 4))
        tk.Label(f, text="Load a diagnostic ZIP to see security and health findings.",
                 bg=C_BG, fg=C_TEXT_DIM, font=("Segoe UI", 10)).pack()

    def _build_health_tab(self):
        """Populate the Health tab from self._health_report."""
        f = self._tab_health
        for w in f.winfo_children():
            w.destroy()
        ...
        tk.Label(hdr, text="\U0001fa7a  Security & Health",
                 bg=C_BG, fg=C_ACCENT, font=("Segoe UI", 14, "bold")).pack(side="left")

        if report:
            n_err  = len(report.errors())
            n_warn = len(report.warnings())
            badge_color = C_ERROR if n_err else (C_WARN if n_warn else C_OK)
            badge_text  = (f"  {n_err} error(s)  |  {n_warn} warning(s)  " if (n_err or n_warn)
                           else "  All checks passed  ")
            tk.Label(hdr, text=badge_text, bg=badge_color, fg="#ffffff",
                     font=("Segoe UI", 9, "bold"), padx=6, pady=2,
                     relief="flat").pack(side="right", padx=4)

        # ── Scrollable body ───────────────────────────────────────────────────
        outer = tk.Frame(f, bg=C_BG)
        ...
        SEV_COLOR  = {"OK": C_OK, "WARN": C_WARN, "ERROR": C_ERROR, "INFO": C_TEXT_DIM}
        SEV_ICON   = {"OK": "\u2705", "WARN": "\u26a0\ufe0f", "ERROR": "\u274c", "INFO": "\u2139\ufe0f"}
        CAT_ICON   = {
            "BitLocker":   "\U0001f512",
            "Defender":    "\U0001f6e1\ufe0f",
            "Performance": "\U0001f4ca",
            "Storage":     "\U0001f4be",
            "Office Auth": "\U0001f510",
            "Drivers":     "\U0001f527",
            "Legacy Apps": "\U0001f9f9",
            "Store Apps":  "\U0001f6d2",
            "Entra Join":  "\U0001f4bb",
        }

        # Collect categories in priority order
        CATEGORY_ORDER = [
            "Entra Join", "BitLocker", "Defender",
            "Performance", "Storage", "Office Auth",
            "Drivers", "Legacy Apps", "Store Apps",
        ]
        cats_seen = []
        for cat in CATEGORY_ORDER:
            if any(f.category == cat for f in report.findings):
                cats_seen.append(cat)
        # Append any unexpected categories
        for f in report.findings:
            if f.category not in cats_seen:
                cats_seen.append(f.category)

        for cat in cats_seen:
            findings = [fi for fi in report.findings if fi.category == cat]
            if not findings:
                continue

            # Worst severity for the category
            sevs = [fi.severity for fi in findings]
            worst = ("ERROR" if "ERROR" in sevs else
                     "WARN"  if "WARN"  in sevs else
                     "OK"    if "OK"    in sevs else "INFO")
            cat_color = SEV_COLOR.get(worst, C_TEXT_DIM)

            # Category header row
            cat_frame = tk.Frame(body, bg=C_BG)
            cat_frame.pack(fill="x", padx=8, pady=(14, 2))
            icon = CAT_ICON.get(cat, "\U0001f4cb")
            tk.Label(cat_frame,
                     text=f"{icon}  {cat}",
                     bg=C_BG, fg=cat_color,
                     font=("Segoe UI", 11, "bold")).pack(side="left")

            # Separator
            sep = tk.Frame(body, bg=C_PANEL, height=1)
            sep.pack(fill="x", padx=8, pady=(0, 4))

            # Finding rows
            for fi in findings:
                row_bg = C_SURFACE
                row = tk.Frame(body, bg=row_bg, padx=8, pady=4)
                row.pack(fill="x", padx=8, pady=2)

                # Icon + title
                fi_color = SEV_COLOR.get(fi.severity, C_TEXT_DIM)
                fi_icon  = SEV_ICON.get(fi.severity, "•")
                title_lbl = tk.Label(row,
                                     text=f"{fi_icon}  {fi.title}",
                                     bg=row_bg, fg=fi_color,
                                     font=("Segoe UI", 9, "bold"),
                ...
                # Value badge (if any)
                if fi.value:
                    tk.Label(row, text=f"  {fi.value}",
                             bg=row_bg, fg=C_TEXT_DIM,
                             font=("Segoe UI", 8), anchor="w").pack(fill="x")

                # Detail text
                if fi.detail:
                    detail_txt = fi.detail if len(fi.detail) < 300 else fi.detail[:297] + "..."
                    tk.Label(row, text=detail_txt,
                             bg=row_bg, fg=C_TEXT,
                             font=("Segoe UI", 8),
                             wraplength=700, anchor="w", justify="left").pack(fill="x", pady=(1, 0))

                # Action text (highlighted)
                if fi.action:
                    act_txt = fi.action if len(fi.action) < 300 else fi.action[:297] + "..."
                    tk.Label(row,
                             text=f"\u2192  {act_txt}",
                             bg=row_bg, fg=C_ACCENT2,
                             font=("Segoe UI", 8, "italic"),
                             wraplength=700, anchor="w", justify="left").pack(fill="x", pady=(2, 0))
