...
class IMERegistryParser:
    ...
    def parse(self, reg_file: str) -> bool:
        ...
        for line in txt.splitlines():
            l = line.strip()
            # Track section for AgentTelemetryMetadata
            if '[' in l and ']' in l:
                in_telemetry = 'AgentTelemetryMetadata' in l
            # IME agent version: first "Version" entry in AgentTelemetryMetadata
            if in_telemetry and not self.agent_version and '"Version"' in l:
                v = self._str_val(l)
                if re.match(r'^\d+\.\d+', v):
