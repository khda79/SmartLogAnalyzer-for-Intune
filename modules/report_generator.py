...
class ReportGenerator:
    ...
    def generate(self,
        ...
        self._add_section("s-c2r", "🏢 Office C2R Logs",
                          self._html_c2r(hardware_parser))
    ...
    # ──────────────────────────────────────────────────────────────
    def _html_c2r(self, hp):
        if not hp or not hp.c2r.parsed or not hp.c2r.log_files:
            return '<div class="alert alert-info">No Office C2R logs found.</div>'

        c2r = hp.c2r
        errors = c2r.parse_all_errors() if hasattr(c2r, "parse_all_errors") else []

        summary = (
            f'<div class="alert alert-info">'
            f'<strong>{len(c2r.log_files)}</strong> C2R file(s) analyzed. '
            f'<strong>{len(errors)}</strong> error(s) detected.</div>'
        )
        if not errors:
            return summary
        ...
        for e in errors[:500]:
            rows += (
                f"<tr><td style='white-space:nowrap'>{_esc(e.timestamp)}</td>"
                f"<td>{_sev_badge(e.level)}</td>"
                f"<td>{_esc(e.process)}</td>"
                f"<td>{_esc(e.message)}</td></tr>"
            )
        return summary + (
            '<div class="card"><div class="card-header">C2R Errors</div>'
            '<div class="card-body">' +
            _tbl([], ["Timestamp","Level","Process","Message"],
                 rows, "tbl_c2r", filterable=True) +
            '</div></div>'
        )
