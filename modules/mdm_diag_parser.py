...
class MDMDiagParser:
    ...
    def _detect_issues(self):
        ...
        if not self.certificates:
            # Check if device shows active MDM management in connection info
            # (e.g., Workplace Join / BYOD enrollments don't provision device certs
            # the same way as AAD Join corporate enrollments — this is a known
            # limitation of MDMDiagnosticsTool, not necessarily an enrollment problem)
            _managed_by = self.connection_info.get('Managed by', '') or \
                          self.connection_info.get('MDM Server', '')
            _last_sync  = self.connection_info.get('Last sync', '') or \
                          self.connection_info.get('Last MDM sync', '')
            if _managed_by or _last_sync:
                # Device appears actively managed — cert absence is likely a
                # Workplace Join / BYOD / co-management reporting artifact
                issues.append(MDMIssue(
                    severity='WARNING',
                    area='Certificates',
                    title='No MDM device certificates visible in this report',
                    detail='No MDM certificate rows found in MDMDiagReport.html. '
                           'This is expected for Workplace Join (BYOD/Personal) enrollments '
                           'where authentication uses AAD user tokens rather than device '
                           'certificates. The device appears actively managed by MDM.',
                    recommendation='Verify enrollment in Intune portal. If the device is '
                                   'BYOD/Personal ownership, this alert can be ignored. '
                                   'For corporate AAD-Joined devices, consider re-enrolling '
                                   'if MDM sync issues are observed.'))
            else:
                issues.append(MDMIssue(
                    severity='ERROR',
                    area='Certificates',
                    title='No Intune MDM certificates found',
                    detail='The device may not be properly enrolled or the MDM certificate '
                           'has expired. No active management connection info was detected.',
